...
「账号与额度」区的每张卡片都用**固定三行**展示额度：`5 小时 / 周 / 月`（顺序固定），
缺的周期也占位显示「不适用」，所以不同订阅的卡片同构、同名行可以横向对齐；
用量区的筛选与汇总也做成了等宽栅格：账号 / 模型 / 项目三个筛选框在宽屏三等分、
窄屏（≤700px）单列堆叠，不再出现「两个一行 + 一个撑满」的参差换行；
汇总卡按 130px 自适应列数（宽屏一行放完 3~5 张），窄屏两列，张数为奇数时
最后一张占满整行，不会留一张孤零零的卡片。金额统一为「≥$1 保留 2 位、
小额保留 4 位」的展示，精确值放在悬停提示里。

卡片之间横向对齐：同一行的卡片等高、表头高度固定（账号 ID 长短不影响），
额度行用固定列宽（`92px / 1fr / 76px`），所以三行窗口与进度条、百分比、重置文案在各卡片之间
严格对齐——`.account-list` 不能用 `align-items: start`、百分比列不能用 `auto`，
`account-meta` 固定两行高度，这几条都有测试钉住。
**没有任何额度窗口的订阅整张卡片都不显示**（例如 DeepSeek Harness、Claude Code），
本区计数与下拉也只算显示出来的订阅；全部订阅都没有额度窗口时给一段空状态说明。
注意这类 provider 的使用量仍计入「用量与成本估算」，只是不出现在额度看板里。
每行的字段固定为「周期 / 进度条 / 百分比 / 重置时间与窗口时长」，三种状态分别是：
有数据（绿 / ≥80% 黄 / 耗尽红）、该订阅没有这个周期（虚线「不适用」）、
上游没返回已用比例（「待采集」）。同一周期有多条窗口时（例如 Kimi 的
`limit_month_total` 与 `limit_month_code`）取最紧的一条做主行，其余用「另有 N 条」
角标提示。周期口径由后端统一给出（`quota_period()`：窗口时长优先、窗口名兜底，
`primary` / `weekly` / `limit_month_total` / `5-hour` / `monthly` 都会归到同一套周期），
命令行 `quota` 也用同一套口径输出「周窗口（…）/ 月窗口（…）」。
...
- 「用量与成本估算」区支持三种统计维度：按账号（默认）、按模型、按项目。
  账号维度里明细表第一列与账号成本排行都用 `产品 · 套餐/账号 ID` 标签
（表头相应地叫「订阅 / 账号 ID」，次要行保留 `Profile：…`），
与「账号与额度」卡片同源、同一套展示名归一化。三种维度由同一份模型级叶子数据聚合，切换维度不改变合计口径：表尾给出合计行，
  「调用模型」列在模型很多时默认只列**用量最大的前 3 个**，其余收在「另有 N 个模型」
按钮里，点开可看全部、再点收起（展开状态在切换维度/刷新后保持）；模型按 token 用量倒序，
所以折叠状态下露出来的就是大头。每行给出在筛选结果内的金额占比，并且始终显示账号成本排行 Top 5（金额 + 占比；标签格式为
`产品 · 套餐/账号 ID`，例如 `Codex · Plus/35946d17-…`，套餐与账号卡片同源）
  与项目成本排行 Top 5——两者**并排两列**（窄屏 900px 以下堆叠为上下排列，
  没有任何可计价金额时整块不渲染，不会留半张空栏）。筛选条件为账号、模型、项目三者可叠加；
...
- 「习惯分析」区对已索引的对话做本地统计分析，可按全部 / 今天 / 近 7 天 /
  近 30 天切换统计窗口（今天按本地日历日 0 点起算，与用量区口径一致）：习惯画像（平均每对话规模、最活跃时段、主力模型、
  用量最高的一天、周末占比、缓存累计节省）、
  活跃时段图、模型成本分布、对话规模分布和最贵对话 Top 5（标签为
  `订阅 · 账号 · 会话 · 项目路径`，例如
  `Grok · SuperGrok · nf7wr5pwr4@… · 会话 0ac7c04e-d35 · /home/lsl/project/…`，
  订阅与账号卡片同源，会话 ID 显示前 12 位、悬停可看完整 ID），并按规则生成
...
内置单价覆盖 GPT-6（`gpt-6-astra` $10/$1/$50、`gpt-6-sol` $2/$0.2/$10、
`gpt-6-luna` $0.1/$0.01/$0.5，均按官方规则对 >272K 输入套用 2x/2x/1.5x）、
小米 MiMo（`mimo-v2.6-flash` 国际站 $0.14/$0.0028/$0.28，聚合商的
`xiaomi/mimo-v2.6-flash` 前缀与 `-RL` 后缀都能命中；官方无长上下文加价，已显式关闭）、
智谱 GLM（`glm-5.3` $1.4/$0.26/$4.4、`glm-5.3-flash` $0.15/$0.03/$0.5、
`glm-5.3-flashx` $0.37/$0.075/$1.25，取 Z.ai 官方美元价目）与阶跃星辰
（`step-5-preview` 官方 ¥7/¥0.35/¥20，按 7.0 折算为 $1/$0.05/$2.86）。
聚合商前缀（`z-ai/…`、`stepfun/…`）、大小写与官方快照后缀都能命中同一份价格。
