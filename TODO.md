...
- [x] 补充新发布模型的定价
  - `gpt-6-sol`（$2 / $0.2 / $10）、`gpt-6-luna`（$0.1 / $0.01 / $0.5）按 OpenAI 官方
    文档录入，含 >272K 输入的 2x/2x/1.5x 长上下文规则。
  - `mimo-v2.6-flash`（国际站 $0.14 / $0.0028 / $0.28）按小米 MiMo 开放平台录入；
    聚合商前缀（`xiaomi/…`）与开源权重命名（`…-RL`）靠同一份 tail/后缀匹配命中；
    官方没有长上下文加价，显式把三个乘数设为 1.0。
  - 面板定价来源增加 `mimo_api_source`，说明 MiMo 按国际站单价估算。

- [x] 账号成本排行标签统一成「产品 · 套餐/账号 ID」
  - 明细表第一列同步改成同一份标签，表头由「账号 / Profile」改为「订阅 / 账号 ID」，
    次要行保留 `Profile：…`；两个排行与表格共用 `accountLabel()` 与一份订阅表
    （renderUsage 里算一次，逐行复用）。
  - 排行行标签改为 `Codex · Plus/35946d17-aa48-…`：产品与套餐取自 `/api/state`
    （与账号卡片同源、同一套展示名归一化），斜杠后的账号 ID 用次要色；
    没有独立账号 ID 的 provider（Kimi）不重复显示 `/kimi`。
...
- [x] 英文版（完成）
  ...
  - [x] CLI 的 `--json` 输出同样按语言本地化（和 `/api/*` 一致，用户数据里的中文路径不动）。
  - [x] 收尾：英文页面里的**中文标点清零**（补 `）`、`：`、`、` 等条目）；
    `use_language('zh')` 会拆掉输出代理，同进程反复切换不留残留。
  - [x] 终验（12 项跨端检查全绿）：中文页面默认中文；英文页面/设置页/接口**无中文、
    无中文标点**；`<html lang>` 正确；CLI `--help`（`--lang en` 与 `LANG=en_US`）
    与 `status --json --lang en` 均无中文，默认仍是中文；605 项测试通过、0 跳过。
...
- [x] 习惯分析支持「今天」窗口
  - 新增 `calendar_day_start()`（本地时区 0 点），`/api/insights?days=today` 走这个起点；
    ...
    （all / days / today）与 `window_since` 标明，窗口名称由前端按同一字段显示。
  - `insights()` 新增 `since` / `window_kind` 参数，`_insights_window()` 统一解析窗口参数。
  - 验证：路由级断言 `days=today` 的 window_since 等于本地日历日起点、`days=1` 仍是
    滚动 24 小时；聚合级断言今天窗口是全部历史的子集；浏览器侧确认 tab 顺序
    （全部 / 今天 / 近 7 天 / 近 30 天）、请求参数与窗口文案随切换一起变。

- [x] 精简所有区块的说明文字，只留一行摘要
  - 「账号与额度」237 → 82 字（去掉卡片标题样式、ID 归并、归档操作等已在界面与
    README 里体现的细节）、「维护」151 → 80 字、「用量检索」96 → 66 字、
    「告警历史」78 → 49 字、「扫描目录」186 → 83 字、「历史数据」135 → 81 字；
    「异常流量监控」90 → 52 字。
  - 折叠区块的说明在 JS 模板里（`<span>`），第一轮只按 `<h2>+<p>` 扫会漏掉，已改成
    按 class 抓全部并加断言（>90 字直接失败）；浏览器侧也会先展开折叠区块再量行数。
  - 实测：主页 @1440/@1120 与设置页 8 条说明全部只占一行。

- [x] 删掉「用量与成本估算」标题下的整段说明文字
  - 该说明（按 Codex/Grok/Kimi/… 账号汇总、三种统计维度、账号 ID 归并口径）
    篇幅长且信息已在界面元素与 README 里体现，按需求从区块标题下移除；
    其余区块的说明文字保持不变，删后标题高度 50px、无残留空行。

- [x] 最贵对话标签改成「订阅 · 账号 · 会话 · 项目路径」
  - 「习惯分析」里的最贵对话原来主标签是裸会话 UUID、补充行只有「账号 · 项目」；
    现在主标签是 `产品 · 套餐`（与账号卡片同源），后面依次接账号、会话、项目路径，
    未定价提示仍缀在末尾。
  - 会话 ID 显示前 12 位（与用量检索 / 归档列表一致），完整 ID 放进整行悬停提示
    （statRow 新增可选 title 参数，其它调用点不受影响）。
  - 产品认不出来时与账号卡片一致兜底成 Codex；账号不在 /api/state 里时退回显示账号名。
  - 验证：真实浏览器断言三行标签文本与 tooltip（含完整会话 ID、无项目、未知账号等退化情况）。
...
- [x] Command Code 活动会话读取
  ...
  - 新增工作目录回退：复刻官方 slugify(cwd) 规则（驼峰拆词 / 小写 / 80 字符截断）
    定位 `~/.commandcode/projects/<slug>`，取进程启动之后修改过、且会话头 cwd 与
    进程一致的最近会话；证据等级记 `recent_file`，句柄命中仍是 `open_file`，
    同一个会话同时满足时按句柄计。
  - 目录名被截断或规则变化时退化为前缀匹配，但会话归属仍由会话头 cwd 二次校验。
  - 验证：slug 规则与机器上 5 个真实项目目录逐一比对；假 /proc 用例覆盖句柄、
    目录回退、过期会话、启动时间守卫、跨目录误匹配、证据优先级；
    另用真实进程（名为 command-code 的假 CLI）跑通「无句柄 → recent_file」、
    「持有句柄 → open_file」、「无进程 → 不报」三种场景。

- [x] 用量区筛选与汇总排版整理
  - 筛选框从 flex-wrap + 固定 220px 改成等宽栅格（宽屏三等分、≤700px 单列），
    不再出现「账号/模型一行、项目撑满第二行」的参差换行。
  - 汇总卡从固定三列改成 130px 自适应列数：1440/1280/1024/860 下 3~5 张都排成一行，
    ≤760px 两列且奇数张时最后一张占满整行，避免孤立卡片。
  - 金额格式统一为「≥$1 两位小数、<$1 四位小数」（原先 API 金额六位小数和
    缓存节省两位小数并排），精确值放进 tooltip。

- [x] 成本排行并排两列
  - 「账号成本排行 Top 5」与「项目成本排行 Top 5」放进同一个两列网格，等高并排；
    窄屏（900px 断点，页面本身也在这里切成单列）自动堆叠为上下排列。
  - 只有一个排行有数据时占满整行（single 类兜底），两者都没有可计价金额时整块不渲染。
  - 实测 1500px：两块 left 294 / 873、宽 565、top 完全一致；820px：同一 left、上下排列。

- [x] 「调用模型」长列表自动折叠
  - 账号维度的模型列默认只显示用量最大的前 3 个，其余用「另有 N 个模型」按钮展开/收起，
    展开状态记录在 Set 里（切换维度、重新渲染后保持），模型按 token 倒序排列。
  - 实测：12 个模型的账号默认行高 105px、展开后显示全部、再次收起行高复原；
    2 个模型的账号不出现折叠按钮。
...
- [x] 额度窗口统一成固定行
  ...
  - 跨卡片对齐：卡片等高（去掉 align-items:start）、表头高度固定（meta 两行封顶 +
    最小高度）、额度行固定列宽（百分比列不再 auto），实测五张卡首行偏移一致 185px、
    卡片高度一致 533px、进度条位置与长度完全一致；页脚用 margin-top:auto 贴底。
